...
def search_venues():
    ...
    venues = Venue.query.filter(Venue.name.ilike(search)| Venue.city.ilike(search)| Venue.state.ilike(search))
    response={
       "count": venues.count(),
       "data": venues
    }
...
def search_artists():
    ...
    search = '%' + request.form['search_term'] + '%'
    artists = Artist.query.filter(Artist.name.ilike(search) | Artist.city.ilike(search)| Artist.state.ilike(search))
    response={
       "count": artists.count(),
       "data": artists
    }
...
def show_artist(artist_id):
    ...
    past_show = db.session.query(Show).join(Artist).filter(Show.artist_id == artist_id, Show.start_time < datetime.now())
    past_shows= []
    for show in past_show:
      past_shows.append({
    ...
    upcoming_show = db.session.query(Show).join(Artist).filter(Show.artist_id == artist_id, Show.start_time >= datetime.now())
    upcoming_shows= []
    for show in upcoming_show:
      upcoming_shows.append({
        "venue_id": show.venue_id,
